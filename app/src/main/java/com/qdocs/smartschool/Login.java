...
//                Log.e("PRIVACY URL", domain);
                String domain="https://schoolingpro.in/privacy_policy.php";
            ...
            public void onClick(View v) {
                ...
                if (username.isEmpty() && password.isEmpty()) {
                    ...
                    if (Utility.isConnectingToInternet(Login.this)) {
                        ...
                        System.out.println("Login Details==" + obj);
                        //  getDataFromApi(this,obj.toString());
                        getDataFromApi(Login.this, obj);
                    } else {
                        makeText(getApplicationContext(), R.string.noInternetMsg, Toast.LENGTH_SHORT).show();
                    }
                }
    ...
    private void getDataFromApi1(String bodyParams) {
        final ProgressDialog pd = new ProgressDialog(this);
        pd.setMessage("Loading");
        pd.setCancelable(false);
        pd.show();
    ...
    public void getDataFromApi(Context context, JSONObject bodyParams) {
        ProgressDialog pd = new ProgressDialog(context);
        pd.setMessage("Loading...");
        pd.setCancelable(false);
        pd.show();

        String url = Utility.getSharedPreferences(context, "apiUrl") + Constants.loginUrl;
        Log.d(TAG, "Request URL: " + url);
        Log.d(TAG, "Request Body: " + bodyParams.toString());

        JsonObjectRequest jsonRequest = new JsonObjectRequest(
                Request.Method.POST, url, bodyParams,
                response -> {
                    pd.dismiss();
                    ...
                    try {
                        ...
                        if (status.equals("1")) {
                            Utility.setSharedPreference(getApplicationContext(), Constants.loginType, response.getString("role"));
                            JSONObject data = response.getJSONObject("record");
                            Log.d("TAG", "LoginReponj " + data);
                            Utility.setSharedPreference(getApplicationContext(), Constants.userId, response.getString("id"));
                            Utility.setSharedPreference(getApplicationContext(), "accessToken", response.getString("token"));
                            Utility.setSharedPreference(getApplicationContext(), "schoolName", data.getString("sch_name"));
                            Utility.setSharedPreference(getApplicationContext(), Constants.currency, data.getString("currency_symbol"));
                            Utility.setSharedPreference(getApplicationContext(), Constants.currency_short_name, data.getString("currency_short_name"));
                            Utility.setSharedPreference(getApplicationContext(), "startWeek", data.getString("start_week"));
                            ...
                            Utility.setSharedPreferenceBoolean(context, "isLoggedIn", false);
                        }
                    } catch (JSONException e) {
                ...
                error -> {
                    ...
                    Toast.makeText(context, "API Error. Please try again.", Toast.LENGTH_LONG).show();
                }) {
            @Override
            public Map<String, String> getHeaders() throws AuthFailureError {
